...
DOCUMENTATION = '''
---
module: oneops_transition_attachment

short_description: A module for provisioning a OneOps attachment inside of a transition
...
def get_oneops_attachment_module():
    return AnsibleModule(
        argument_spec=module_argument_spec.get_oneops_transition_attachment_module_argument_spec(),
        supports_check_mode=True,
    )


def ensure_transition_attachment(module, state):
    old_attachment = dict()
    ...
    # Collect unreleased changes if attachment is was updated
    atts_diff = None
    if new_attachment['rfcAction'] == 'update':
        atts_diff = dict_transformations.recursive_diff(old_attachment['ciBaseAttributes'],
                                                        new_attachment['ciBaseAttributes'])
...
def run_module():
    ...
    if module.params['attachment']['state'] == 'present':
        return ensure_transition_attachment(module, state)
